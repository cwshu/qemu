...
static void wgchecker_iommu_notify_all(RISCVWgCheckerState *s)
{
    ...
    for (int i = 0; i < WGC_NUM_REGIONS; i++) {
        ...
        uint32_t nworlds = world_config->nworlds;
...
static bool wgc_skip_check(RISCVWgCheckerState *s, uint32_t wid)
{
    if (wid == world_config->nworlds) {
        /*
         * "wid == nworlds" is a WID specific to QEMU.
         * It means unspecified attribute transactions. Skip them to support
         * QEMU '-bios' and '-device loader' options to load firmwares into
         * the memory protected by the wgchecker.
         */
        return true;
    }
    ...
    RISCVWgCheckerState *s, hwaddr phys_addr, uint32_t wid, bool is_write)
{
    ...
    if (wgc_skip_check(s, wid)) {
        result.is_success = true;
        result.perm = P_READ | P_WRITE;
        return result;
    }
...
static void riscv_wgchecker_realize(DeviceState *dev, Error **errp)
{
    ...
    if (world_config == NULL) {
        error_setg(errp, "Couldn't find global World configs. "
...
static void riscv_wgchecker_reset_enter(Object *obj, ResetType type)
{
    ...
    if (s->num_default_slots != 0) {
        ...
        memcpy(&s->slots[1], s->default_slots,
               ...
               ((world_config != NULL) && world_config->hw_bypass)) {
        /* HW bypass mode */
