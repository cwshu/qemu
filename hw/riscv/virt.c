                                   ...
                                   uint32_t *intc_phandles)
{
    ...
    for (cpu = s->soc[socket].num_harts - 1; cpu >= 0; cpu--) {
        ...
        if (object_property_get_bool(OBJECT(s), "world", NULL)) {
                              ...
                              DeviceState *irqchip)
{
    ...
    /* Enable World extension of each CPU */
    CPU_FOREACH(cpu) {
...
static bool virt_get_world(Object *obj, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);

    return tcg_enabled() && s->have_world;
}

static void virt_set_world(Object *obj, bool value, Error **errp)
{
    RISCVVirtState *s = RISCV_VIRT_MACHINE(obj);
...
static void virt_machine_class_init(ObjectClass *oc, const void *data)
{
    ...
    object_class_property_add_bool(oc, "world", virt_get_world,
                                   virt_set_world);
    object_class_property_set_description(oc, "world",
